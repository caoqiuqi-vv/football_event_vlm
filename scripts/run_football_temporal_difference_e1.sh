...
CONFIG="${CONFIG:-configs/football/dinov3_vitl16_temporal_difference_delta2_from_A_best_16f_hr.yaml}"
INIT_CHECKPOINT="${INIT_CHECKPOINT:-outputs/football_events/vitl16_strong_lora12_mlp_16f_hr_e1/best.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/football_events/vitl16_temporal_difference_delta2_from_A_best_16f_hr}"
...
TARGET_BACKBONE_LR="${TARGET_BACKBONE_LR:-0.000002}"
EPOCHS="${EPOCHS:-4}"
...
{
  echo
  echo "===== PDF E1: z + delta-z + delta2-z residual temporal input ====="
  ...
  echo "dino_lora_frozen=true temporal_difference=delta2 gate_init=0.25"
...
CUDA_VISIBLE_DEVICES="${GPU_LIST}" PYTHONUNBUFFERED=1 "${PYTHON_BIN}" train_football_events.py \
  ...
  "model.temporal_difference.enabled=true" \
  "model.temporal_difference.mode=delta2" \
  "model.temporal_difference.gate_init=0.25" \
  "train.epochs=${EPOCHS}" \
