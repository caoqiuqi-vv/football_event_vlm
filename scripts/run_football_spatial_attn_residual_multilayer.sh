...
CONFIG="${CONFIG:-configs/football/dinov3_vitl16_spatial_attn_residual_multilayer_16f_hr.yaml}"
BASE_INIT_CHECKPOINT="${INIT_CHECKPOINT:-outputs/football_events/vitl16_temporal_spatial_attention_v2_region_only_16f_hr/best.pt}"
WARMUP_OUTPUT_DIR="${WARMUP_OUTPUT_DIR:-outputs/football_events/vitl16_spatial_attn_residual_multilayer_16f_hr_warmup}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/football_events/vitl16_spatial_attn_residual_multilayer_16f_hr}"
...
WARMUP_HEAD_LR="${WARMUP_HEAD_LR:-0.0001}"
...
run_stage() {
  ...
  {
    echo
    echo "===== residual multi-layer spatial attention: ${stage} ====="
    ...
    echo "feature_readout=loaded_2048_reference+zero_init_multilayer_residual layers=-8,-4,-1"
    echo "spatial_patch=loaded_last_layer+zero_init_multilayer_residual mil_balance=per_class_equal_pos_neg"
  } | tee -a "${log_file}"
