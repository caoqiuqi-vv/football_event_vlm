#!/usr/bin/env python
"""Verify aligned event loss directly updates the exact v5 attention map."""
...
from train_football_events_featuremap_r1_v5 import (
    load_featuremap_r1_v5,
    load_v5_config,
)
...
def main() -> None:
    ...
    cfg = load_v5_config(args.config, [])
    cfg.gpu_ids = [0]
    cfg.device = "cuda:0"
    ...
    model = load_featuremap_r1_v5(cfg, torch.device(cfg.device))
    model.eval()
    ...
    print(
        "FEATUREMAP_R1_V5_SMOKE_OK",
        {
            ...
            "mil_weight": float(cfg.train.spatial_attention_mil_loss_weight),
        },
        flush=True,
    )
