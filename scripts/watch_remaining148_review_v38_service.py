#!/usr/bin/env python3
"""Keep the public remaining148 v38 review service healthy on port 8775."""
...
PORT = 8775
SESSION = "football_remaining148_review_v38_server_20260908"
PROXY_ENV_NAMES = (
...
def stop_expected_previous_server() -> None:
    ...
    if "server_multiuser_v38.py" in command and expected_db in command:
        return
    previous_versions = (
        "server_multiuser_v37.py",
        "server_multiuser_v36.py",
        "server_multiuser_v35.py",
    )
...
def start_server() -> None:
    command = [
        ...
        "tools/football_event_review/server_multiuser_v38.py",
    ...
    if result.returncode:
        raise RuntimeError("failed to start v38 server tmux")
...
def wait_health() -> None:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if json.loads(data).get("version") == "v38-targeted-range-team-drawer":
                return
        except Exception as error:
            last_error = error
        time.sleep(1)
    raise RuntimeError(f"v38 direct health check failed: {last_error}")
...
def main() -> None:
    ...
    while True:
        ...
        ready = {
            "status": "ready_public_targeted_http11_range",
            "version": "v38-targeted-range-team-drawer",
