...
CONFIG="${CONFIG:-configs/football/dinov3_vitl16_er1_multilayer_readout_16f_hr_from_scratch_20ep.yaml}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/football_events/vitl16_er1_multilayer_readout_16f_hr_from_scratch_20ep}"
BACKBONE_WEIGHTS="${BACKBONE_WEIGHTS:-checkpoints/dinov3_vitl16_pretrain_lvd1689m-8aa4cbdd.pth}"
...
RESUME_LOAD_OPTIMIZER="${RESUME_LOAD_OPTIMIZER:-true}"
RESUME_LOAD_SCHEDULER="${RESUME_LOAD_SCHEDULER:-true}"
RESUME_LOAD_SCALER="${RESUME_LOAD_SCALER:-true}"
...
{
  echo
  echo "===== E-R1 from-scratch multi-layer DINO readout 16f 640x1120 ====="
  echo "started_at=$(date --iso-8601=seconds)"
  echo "physical_gpus=${GPU_LIST} local_gpu_ids=${local_gpu_ids}"
  echo "config=${CONFIG}"
  echo "init_checkpoint=<none: DINO pretrained only>"
