...
CONFIG="${CONFIG:-configs/football/dinov3_vitl16_dual_roi_shared_16f_hr.yaml}"
INIT_CHECKPOINT="${INIT_CHECKPOINT:-outputs/football_events/vitl16_robust_dual_16f_exp4/best.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/football_events/vitl16_dual_roi_shared_16f_hr}"
...
{
  echo
  echo "===== shared-backbone dual-ROI 16-frame experiment ====="
  echo "started_at=$(date --iso-8601=seconds)"
  echo "physical_gpus=${GPU_LIST}"
  echo "config=${CONFIG}"
  echo "init_checkpoint=${INIT_CHECKPOINT}"
  echo "annotations_dir=/home/new_users/qiuqi/code/football_events_human_repair"
  echo "views=global_512x896+roi_a_384x640+roi_b_384x640 frames=16 aligned"
