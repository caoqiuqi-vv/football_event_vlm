#!/usr/bin/env python
"""Verify classification updates v7 only through actual attention contrast."""
...
from train_football_events_featuremap_r1_v7 import (
    load_featuremap_r1_v7,
    load_v7_config,
)
...
def main() -> None:
    ...
    cfg = load_v7_config(args.config, [])
    cfg["gpu_ids"] = [0]
    cfg["device"] = "cuda:0"
    ...
    model = load_featuremap_r1_v7(cfg, torch.device(cfg.device))
    model.eval()
