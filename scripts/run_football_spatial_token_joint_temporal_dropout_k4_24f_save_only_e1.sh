...
CONFIG="${CONFIG:-configs/football/dinov3_vitl16_spatial_token_joint_temporal_dropout_k4_24f_save_only_e1.yaml}"
INIT_CHECKPOINT="${INIT_CHECKPOINT:-outputs/football_events/vitl16_spatial_token_joint_temporal_k4_24f_hr/epoch_1.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/football_events/vitl16_spatial_token_joint_temporal_dropout_k4_24f_save_only_e1}"
...
TARGET_BACKBONE_LR="${TARGET_BACKBONE_LR:-0.0}"
EPOCHS="${EPOCHS:-3}"
...
{
  echo
  echo "===== K4 joint spatial-temporal fusion with modality dropout, 24 frames ====="
  ...
  echo "workers_per_gpu=${NUM_WORKERS_PER_GPU} dino_lora_frozen=true spatial_token_pooling=k4 fusion_mode=joint_token_temporal_dropout dino_patch_layers=-8,-4,-1 relation_layers=2"
...
CUDA_VISIBLE_DEVICES="${GPU_LIST}" PYTHONUNBUFFERED=1 "${PYTHON_BIN}" train_football_events.py \
  ...
  "train.resume.load_optimizer=${RESUME_TRAINING}" \
  "train.resume.load_scheduler=${RESUME_TRAINING}" \
  "train.resume.load_scaler=${RESUME_TRAINING}" \
