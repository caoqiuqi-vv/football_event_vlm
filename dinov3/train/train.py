...
def build_data_loader_from_cfg(
    ...
    if data_config_path:
        ...
        global_height, global_width = (int(value) for value in data_config["global_size"])
    else:
        global_height = global_width = int(img_size)
    patch_size = int(cfg.student.patch_size * cfg.crops.teacher_to_student_resolution_scale)
    token_height = global_height // patch_size
    token_width = global_width // patch_size
    n_tokens = token_height * token_width
    mask_generator = MaskingGenerator(
        input_size=(token_height, token_width),
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        # WebDataset handles distributed node and worker splitting itself.
        sampler_type = None
    else:
