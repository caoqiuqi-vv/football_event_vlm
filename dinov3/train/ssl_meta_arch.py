...
from dinov3.data.football_ssl import FootballDataAugmentationDINO, load_football_ssl_data_config
...
class SSLMetaArch(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.lora_enabled = bool(lora_cfg and lora_cfg.get("enabled", False))
        if self.lora_enabled:
            student_injected = inject_lora(student_backbone, lora_cfg)
            teacher_injected = inject_lora(teacher_backbone, lora_cfg)
            if student_injected != teacher_injected:
                raise RuntimeError(
                    f"Student/teacher LoRA mismatch: {student_injected} != {teacher_injected}"
                )
            logger.info(
                "Injected SSL LoRA into %d modules; rank=%d last_blocks=%d",
                student_injected,
                int(lora_cfg.get("rank", 16)),
                int(lora_cfg.get("target_last_blocks", 12)),
            )
    ...
    def build_data_augmentation_dino(self, cfg):
        data_config_path = cfg.train.get("data_config")
        if data_config_path:
            data_config = load_football_ssl_data_config(data_config_path)
            if int(data_config.get("local_crops_number", 4)) != int(cfg.crops.local_crops_number):
                raise ValueError("football data local_crops_number must match crops.local_crops_number")
            return FootballDataAugmentationDINO(
                data_config, mean=cfg.crops.rgb_mean, std=cfg.crops.rgb_std
            )
    ...
    def get_maybe_fused_params_for_submodel(self, name: str, m: nn.Module):
        ...
        lr_multiplier = float(
            self.cfg.optim.get(f"{name}_lr_multiplier", 1.0)
        )
        wd_multiplier = float(
            self.cfg.optim.dino_head_wd_multiplier
            if name == "dino_head"
            else self.cfg.optim.get(f"{name}_wd_multiplier", 1.0)
        )
        for group in params_groups:
            group["lr_multiplier"] *= lr_multiplier
            group["wd_multiplier"] *= wd_multiplier
        logger.info(
            "OPTIM -- %s -- lr_multiplier: %.4f, wd_multiplier: %.4f",
            name,
            lr_multiplier,
            wd_multiplier,
        )
    ...
    def get_params_groups(self):
        ...
        for name, m in self.student.items():
            ...
            all_params_groups += self.get_maybe_fused_params_for_submodel(name, m)
