#!/usr/bin/env python3
"""Four-GPU DDP trainer for the strict no-ROI/no-NMS DINOv3 verifier."""
...
def main() -> None:
    ...
    enabled_backbone = {name for name, value in backbone.named_parameters() if value.requires_grad}
    ...
    for epoch in range(args.epochs):
        if epoch == args.warmup_epochs:
            for name, parameter in model.module.backbone.named_parameters():
                parameter.requires_grad_(name in enabled_backbone)
        ...
        for batch in loader:
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                if positive.any():
                    loss = loss + 2 * F.smooth_l1_loss(result["time_delta_sec"][positive], batch["delta"][positive].clamp(-2, 2))
        ...
        if rank == 0:
            torch.save({
                ...
                "world_size": dist.get_world_size(), "verifier_variant": "full_image_only", "roi_branch": False,
            }, output / "last.pt")
