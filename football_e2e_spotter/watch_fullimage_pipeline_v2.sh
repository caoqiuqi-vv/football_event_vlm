#!/usr/bin/env bash
# End-to-end watchdog: verified baseline -> no-NMS Stage-1 -> full-image verifier.
set -euo pipefail
...
mkdir -p "$work"
echo "$(date -Is) fullimage_pipeline_v2_watchdog_started" >> "$log"
...
CUDA_VISIBLE_DEVICES=0,1,2,7 PYTHONUNBUFFERED=1 PYTHONPATH=.:football_e2e_spotter/src \
  torchrun --standalone --nproc_per_node=4 football_e2e_spotter/train_set_verifier_fullimage_checkpointed_ddp_v3.py \
  --candidates "$work/oof/candidates.jsonl" \
  --annotations /mnt/data_16t/football/football_events_human_repair \
  --output "$work/verifier_fullimage_checkpointed_v3" --epochs 6 --frame-chunk-size 4 >> "$log" 2>&1
[[ -s "$work/verifier_fullimage_checkpointed_v3/last.pt" ]] || { echo "$(date -Is) verifier_missing_checkpoint" >> "$log"; exit 30; }
...
CUDA_VISIBLE_DEVICES=0,1,2,7 PYTHONUNBUFFERED=1 PYTHONPATH=.:football_e2e_spotter/src \
  ...
  --annotations /mnt/data_16t/football/football_events_human_repair \
  --verifier "$work/verifier_fullimage_checkpointed_v3/last.pt" \
  --output "$work/calibration_fullimage_checkpointed_v3" >> "$log" 2>&1
[[ -s "$work/calibration_fullimage_checkpointed_v3/calibration_eval.json" ]] || { echo "$(date -Is) missing_calibration_eval" >> "$log"; exit 40; }
