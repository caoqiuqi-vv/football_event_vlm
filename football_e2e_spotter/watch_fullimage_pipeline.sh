#!/usr/bin/env bash
# End-to-end watchdog: baseline -> Stage-1 no-NMS -> full-image verifier.
set -euo pipefail
...
mkdir -p "$work"
echo "$(date -Is) fullimage_pipeline_watchdog_started" >> "$log"
...
if [[ ! -s "$baseline/last.pt" || ! -s "$baseline/best.pt" ]]; then
  echo "$(date -Is) baseline_failed_or_incomplete: missing last.pt/best.pt" >> "$log"
  exit 10
fi
if ! python -c 'import sys,torch; c=torch.load(sys.argv[1],map_location="cpu",weights_only=False); sys.exit(0 if int(c.get("epoch",0)) >= 8 else 1)' "$baseline/last.pt"; then
  echo "$(date -Is) baseline_failed_or_incomplete: last checkpoint epoch below 8" >> "$log"
  exit 11
fi
...
echo "$(date -Is) verifier_start full_image_only checkpointed" >> "$log"
CUDA_VISIBLE_DEVICES=0,1,2,7 PYTHONUNBUFFERED=1 PYTHONPATH=.:football_e2e_spotter/src \
  torchrun --standalone --nproc_per_node=4 football_e2e_spotter/train_set_verifier_fullimage_checkpointed_ddp_v2.py \
  --candidates "$work/oof/candidates.jsonl" \
  --annotations /mnt/data_16t/football/football_events_human_repair \
  --output "$work/verifier_fullimage_checkpointed_v2" --epochs 6 --frame-chunk-size 4 >> "$log" 2>&1
[[ -s "$work/verifier_fullimage_checkpointed_v2/last.pt" ]] || { echo "$(date -Is) verifier_missing_checkpoint" >> "$log"; exit 30; }
...
CUDA_VISIBLE_DEVICES=0,1,2,7 PYTHONUNBUFFERED=1 PYTHONPATH=.:football_e2e_spotter/src \
  ...
  --annotations /mnt/data_16t/football/football_events_human_repair \
  --verifier "$work/verifier_fullimage_checkpointed_v2/last.pt" \
  --output "$work/calibration_fullimage_checkpointed_v2" >> "$log" 2>&1
[[ -s "$work/calibration_fullimage_checkpointed_v2/calibration_eval.json" ]] || { echo "$(date -Is) missing_calibration_eval" >> "$log"; exit 40; }
