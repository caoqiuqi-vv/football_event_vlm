#!/usr/bin/env bash
# Stable fast profile: batch=3 is memory-safe; DDP unused-parameter detection is
# required because several loaded parameters do not participate in clip loss.
set -euo pipefail
...
run_log="$baseline/resume20_fast_b3a7_stable_console.log"
mkdir -p "$work"

echo "$(date -Is) stable_resume_start batch=3 accum=7 find_unused=true target_epoch=20" >> "$watch_log"
